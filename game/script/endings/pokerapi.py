...
class Dealer:
        ...
        while not self.isBettingOver(checks):
            if self.alivePlayers[currPerson]:
                ...
                match playerDecision[0]:
                    ...
                    case 1:
                        self.checks[currPerson] = True
                        self.contributions[currPerson] += checkVal
                        self.mPlayer[currPerson].changeChips(-checkVal)
                        self.pot += checkVal
                    case 2:
                        for i in checks:
                            checks = False
                        ...
                        self.contributions[currPerson] += checkVal + \
                            playerDecision[1]
                        self.mPlayer[currPerson].changeChips(
    ...
    def helperDraw(cards):
        return max(cards, key=lambda x: 14 if x[0] == 1 else x[0])

    def determineWinners(highestCards, playerIndicies):
        max = -1
        winners = []
        for i in range(len(highestCards)):
            if highestCards[i] > max:
                max = highestCards[i]
                winners = [playerIndicies[i]]
            if highestCards[i] == max:
                winners.append(playerIndicies[i])
        return winners
    ...
    def playRound(self):
        ...
        self.mPlayers[self.mButton].changeChips(-self.smallBlind * 2)
        self.contributions[self.mButton] += self.smallBlind * 2
        self.mPlayers[self.mButton-1].changeChips(-self.smallBlind)
        self.contributions[self.mButton] += self.smallBlind

        self.betting(lastBet=self.SBlind)
        ...
        for winner in winners:
            self.mPlayers[winner].changeChips(self.pot/len(winners))

        self.Deck += self.communityCards
...
class Player:
    ...
    def getAction(communityCards, myCards, checkVal, pot):
